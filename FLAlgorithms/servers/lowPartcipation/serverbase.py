...
class Server:
    ...
    def select_users(self, round, num_users, return_idx=False):
        '''Selects users for a task, prioritizing those with lower participation counts and prints their participation in the previous round.'''
        ...
        # If it's beyond the first round, sort users by their participation count
        if round > 0:
            sorted_users_by_participation = sorted(
                self.users, key=lambda user: self.user_participation_count.get(user.id, 0))
        else:
            # In the first round, simply select users without prioritizing based on participation
            sorted_users_by_participation = list(self.users)[:num_users]

        # Select users based on the sorted order or the initial list
        selected_users = sorted_users_by_participation[:num_users]
        ...
        for user in selected_users:
            ...
            self.user_participation_rounds[user_id].append(round)
             # Calculate previous round's participation if available
            previous_round_participation = self.user_participation_rounds[user_id].count(round - 1) if round > 0 else 0
            print(
                f"User {user_id}: Participation count updated to {self.user_participation_count[user_id]} for round {round}. Previous round participation: {previous_round_participation}")

        # Print the selected users
        selected_user_ids = [user.id for user in selected_users]
        print(f"Round {round}: Selected User IDs: {selected_user_ids}")
