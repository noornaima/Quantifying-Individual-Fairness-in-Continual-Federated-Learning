...
class Server:
    ...
    def select_users(self, round, num_users, return_idx=False):
        '''Selects clients for a round with one client being random and others fixed'''

        num_users = min(num_users, len(self.users))
        active_users_list = list(self.users)

        selected_users = np.random.choice(active_users_list, num_users, replace=False)

        # Update the participation count for each selected user
        ...
        selected_user_ids = [user.id for user in selected_users.tolist()]
        print(f"Round {round}: Selected User IDs: {selected_user_ids}")

        if return_idx:
            selected_indices = [self.users.index(user) for user in selected_users.flatten() if user in self.users]
