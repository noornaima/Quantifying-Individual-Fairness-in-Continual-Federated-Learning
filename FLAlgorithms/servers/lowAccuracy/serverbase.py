...
class Server:
    ...
    def select_users(self, round, num_users, return_idx=False):
        '''Selects users with the lowest accuracies from the previous round.'''
        ...
        if round > 0:
            # Call calculate_class_accuracies and ensure it returns total_acc as a dictionary
            # total_acc, _, _, _ = self.calculate_class_accuracies()
            # Ensure total_acc is a dictionary with user IDs as keys and accuracies as values
            sorted_users = sorted(self.users,
                                  key=lambda x: self.total_accuracies.get(x.id, 0))  # Use .get to avoid KeyErrors
            selected_users = sorted_users[:num_users]
        else:
            # Random selection if it's the first round
            selected_users = np.random.choice(self.users, num_users, replace=False).tolist()

        # Update the participation count for each selected user and print their accuracies
        for user in selected_users:
            ...
            # Print user's participation count and accuracy
            user_accuracy = self.total_accuracies.get(user_id, "N/A") if round > 0 else "N/A (random selection)"
            print(
                f"User {user_id}: Participation count updated to {self.user_participation_count[user_id]} in round {round}, Accuracy: {user_accuracy}")
